...
def naiveSoftmaxLossAndGradient(
    ...
    y_hat_vec = softmax(theta)
    y = np.zeros_like(y_hat_vec)
    y[outsideWordIdx] = 1
    d_theta = y_hat_vec - y
    loss = -np.log(y_hat_vec)[outsideWordIdx]
    gradCenterVec = outsideVectors.T @ d_theta
    # note: can use np.outer
    gradOutsideVecs = (centerWordVec.reshape(-1, 1) @ d_theta.reshape(1, -1)).T
...
def negSamplingLossAndGradient(
    ...
    theta = outsideVectors[indices] @ centerWordVec
    theta[1:] *= -1
    sig = sigmoid(theta)
    loss = -np.log(sig).sum()
    d_theta = 1-sig
    d_theta[0] *= -1
    gradCenterVec = outsideVectors[indices].T @ d_theta

    gradOutsideVecs = np.zeros_like(outsideVectors)
    for i, idx in enumerate(indices):
        gradOutsideVecs[idx] += d_theta[i] * centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        l, d_v_c, d_u = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += l
        gradCenterVecs[centerIdx] += d_v_c
        gradOutsideVectors += d_u
    ### END YOUR CODE
