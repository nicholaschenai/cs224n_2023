...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        seq_unpacked, lens_unpacked = pad_packed_sequence(enc_hiddens)
        enc_hiddens = torch.permute(seq_unpacked, (1, 0, 2))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # Note: einsum cos cleaner than expanding dims for bmm then squeezing. proper way: e_t =
        # torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(2)).squeeze(2)
        e_t = torch.einsum('bh,bmh->bm', dec_hidden, enc_hiddens_proj)
        ...
        a_t = torch.einsum('bm, bmi->bi', alpha_t, enc_hiddens)
        # proper way a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)
